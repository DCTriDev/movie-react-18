...
function AddMovie() {
  ...
  const handleAddNewMovie = (values) => {
    ...
    adminService.insertMovie(newValues).then((res) => {
  ...
  useEffect(() => {
    adminService.getAllActor().then((res) => {
  ...
  const ModalForm = ({ visible, onCancel }) => {
    ...
    return (
      <ModalCustom title={null} footer={null} visible={visible} onOk={onOk} onCancel={onCancel}>
        <FormCustom form={form} layout='vertical' name='sourceForm'>
          ...
          <Form.Item name='detailSource' label='Detail Source'>
          ...
          >
              ...
              rules={[
                {
                  validator: (_, value) => {
                    return validator.isURL(value)
                      ? Promise.resolve()
                      : Promise.reject('Wrong URL format!')
                  },
            ...
            <Form.Item
              ...
              rules={[
                {
                  validator: (_, value) => {
                    return validator.isNumeric(value, 'vi-VN')
                      ? Promise.resolve()
                      : Promise.reject('Wrong number format!')
                  },
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: 'Please choose at least one actor!',
                },
              ]}
            >
            ...
            <Form.Item label='Movie Status' name='movieStatusId' initialValue={1}>
            ...
            <Form.Item label='Movie Type' name='movieTypeId' initialValue={1}>
              <Select className='text-right' theme='dark'>
              ...
              rules={[
                {
                  validator: () => {
                    return sourceList.length > 0
                      ? Promise.resolve()
                      : Promise.reject('No source found!')
                  },
                  ...
                  <ul className='m-0 py-2 rounded-[2px]' id='source-area'>
                    {sourceData.map((source, index) => (
                      <li key={index} className='source text-white list-none'>
                        <DatabaseOutlined />
                        <span className='text-text-color-title ml-2 mr-4'>
                                                    {source.detailSource}
                                                </span>
                        <a
            ...
            <div className='w-full flex justify-end space-x-10'>
              <ButtonPrimary type='submit' className='mr-10'>
                Submit
              </ButtonPrimary>
            </div>
